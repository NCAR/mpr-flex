...
subroutine getMapData(fname,          &   ! input: file name
  ...
  if(ierr/=0)then; message=trim(message)//'problem allocating mapdata%var '; return; endif
  var:do iVar=1,size(mapdata_meta)
    ...
    select case(mapdata_meta(iVar)%varName)
      case('hru_id')
        ...
        if(ierr/=0)then; message=trim(message)//'problem allocating hru_id for mapdata data structure'; return; endif
        ! get the data
      ...
      case('weight')
        ...
        if(ierr/=0)then; message=trim(message)//'problem allocating weight for mapdata data structure'; return; endif
        ! get the data
        ierr = nf90_get_var(ncid, ivarID, mapdata(ix)%var(ivar)%dvar2)
      case('overlapPolyId')
        ! allocate space for the 2D integer array 
        allocate(mapdata(ix)%var(ivar)%ivar2(nOverPoly,nHru),stat=ierr)
        if(ierr/=0)then; message=trim(message)//'problem allocating overlapID for mapdata data structure'; return; endif
        ! get the data
