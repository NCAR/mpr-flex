...
subroutine mpr(ctlfile,    &     ! control file listing geophysical data, mapping data etc
               ParMxyMz,   &     ! Parameter values stored in data type ParMxyMz(:)%var(:,:)
               err, message)

   use globalData,             only:gammaSubset
   use globalData,             only:betaInGamma
  ! MPR core
   USE soiltf                                                     ! Including Soil model parameter transfer function
   USE vegtf,                  only:comp_veg_model_param          ! Including Veg model parameter transfer function
   USE pedotf,                 only:comp_pedotf                   ! Including pedo transfer function and computing module
   ...
   USE def_soiltfParm,         only:read_soil_tfp                 ! Public variables- transfer function coefficients for soil parameter 
   USE def_slyrmod                                                ! Public variables- soil layer multipliers 
   USE def_vegtfParm,          only:read_veg_tfp                  ! Public variables- transfer function coefficients for veg paramger 
   USE def_pedotfParm,         only:read_pedotfParm               ! Public variables- pedotransfer function coefficients 
  ! Ascii I/O
   ...
   USE read_soildata,          only:read_scls                     ! routine to read soil class - properties lookup table 
   USE read_soildata,          only:mod_hslyrs                    ! routine to modify soil layer thickness and updata soil data structure
   USE read_soildata,          only:map_scls2prp                  ! routine to get soil properties from soil class based on lookup table 
  ...
  ! Named variable index 
   ...
   ! input
   character(len=strLen),intent(in)  :: ctlfile                  ! name of the control file
   ! output 
   type(dat_d2d),intent(out)         :: ParMxyMz(:)              ! storage of model soil parameter for 2D field (2nd variable)-model hru x model layer
   ...
   integer(i4b)                      :: iMLyr                    ! index of model soil layer (1,...,n from top to bottom) 
   ...
   integer(i4b)                      :: ixModelDepth             ! generic index variable for model layer depth
   integer(i4b)                      :: ixModelThick             ! generic index variable for model layer depth
   ! mapping stuff  
   ...
   err=0; message='mpr/'
   ! (0.0) Read control file... to obtain location of geohysical data and mapping data
   call get_control(ctlfile, ierr, message)
   if(ierr/=0)then; message=message//cmessage; return; endif 
   ! (0.1) separate beta parameter into veg and soil 
   ! (0.2) get number of soil and vege parameters 
   nSoilParModel=size(soilList)
   nVegeParModel=size(vegeList)  ! this is placeholder
  !  ixModelThick  = ixSoilParVic%h
  !  ixModelDepth  = ixSoilParVic%z
  ! (1) Get Geophysical data 
  ! *****
  ! (1.1) soil data  
  ! *********************************************
  ! (1.1.1) Read in Soil data netCDF...
   allocate(soilClass(nSclass),stat=ierr);if(ierr/=0) call handle_err(ierr,'err allocating for soil class array')
   allocate(scls2prp(nSclass),stat=ierr); if(ierr/=0) call handle_err(ierr,'err allocating for soil properties data type')
   do iSclass=1,nSclass
     allocate(scls2prp(iSclass)%var(nPrpSoil), stat=ierr); if(ierr/=0) call handle_err(ierr,'err allocating for scls2prp%var')
   enddo
   call getData(trim(input_dir)//trim(fname_soil),  & ! input: soil data (netCDF)
  ...
  ! (1.1.2) modify soil layer thickness in sdata data structure
   call mod_hslyrs(sdata,ierr,cmessage); call handle_err(ierr,cmessage)
  ! (1.1.2a) Read in soil class lookup table
   call read_scls(trim(input_dir)//trim(sclass_table), &
                  nSclass,                              &
                  soilClass,                            &
                  scls2prp,                             &
                  ierr, cmessage)
  ...
  ! (1.2.1) Read in veg data netCDF...
   ...
   enddo
   call getVegData(trim(input_dir)//trim(fname_veg), & ! input: file name
  ...
  ! (1.2.2) Read in veg class-property lookup table 
   call getvegClassLookup(trim(input_dir)//trim(vclass_table), &
                          nVclass,                             &
                          vegClass,                            &
                          vcls2prp,                            &
                          ierr, cmessage)
  ...
  ! *********************************************
   ! (2.1) mapping soil polygon to model hru 
    call getMapData(trim(input_dir)//trim(fname_smapping), &   ! input: file name
    ...
    call getMapData(trim(input_dir)//trim(fname_vmapping), &   ! input: file name
  ...
  ! *********************************************
    ...
    enddo
    do iVarHru=1,nVarHru
      call get_vec_dvar(trim(input_dir)//trim(fname_smapping),hru_meta(iVarHru)%varname,hruPrp(iVarHru)%dat,1,nHru,ierr,cmessage); call handle_err(ierr,cmessage)
    enddo
...
! routine to allocate local hru data structures
subroutine allocateLocal(dsNamevar, nHru, nlyr, err, message)
  use var_lookup,     only:ixVarSoilData,nVarSoilData   ! index of soil data variables and number of variables 
  implicit none

  type(namevar), intent(inout)  :: dsNamevar(:)  ! allocate data structure called namevar 
  integer(i4b),  intent(in)     :: nHru          ! list of ID (=index) of soil polygons contributing model hru
  integer(i4b),  intent(out)    :: nlyr          ! soil data container for local soil polygon 
  integer(i4b)                  :: ivar 
  integer(i4b)                  :: iPoly

  ! Initialize error control
  err=0; message='allocateLocal/'

  allocate(sdataLocal(nVarSoilData),stat=ierr); 
  if(ierr/=0) message=trim(message)//'error allocating for sdataLocal'

  do ivar=1,nVarSoilData
    do iPoly=1,nPolyLocal
      select case(sdata_meta(iVarSoil)%vartype)
        case('integer')
          select case(sdata_meta(iVar)%vardims)
            case('2D')
              ! allocate space for the 2D integer array 
              allocate(sdata(ivar)%ivar2(nSlyrs,nPolyLocal),stat=ierr)
            ...
            case('vector')
              ...
              allocate(sdata(ivar)%ivar1(nPolyLocal),stat=ierr)
        ...
        case('double')
          select case(sdata_meta(iVar)%vardims)
            case('2D')
              ! allocate space for the 2D integer array 
              allocate(sdata(ivar)%dvar2(nSlyrs,nPolyLocal),stat=ierr)
              sdataLocal(ivar)%dvar2(:,iPoly)=sdata(ivar)%dvar2(:,polyLocal(iPoly))
            case('vector')
              ! allocate space for the 1D integer array 
              allocate(sdata(ivar)%dvar1(nPolyLocal),stat=ierr)
  ...
  return
end subroutine allocateLocal 

! routine to subset entire domain soil data structures to local hru domain 
subroutine subSoilData(sdata, polyLocal, sdataLocal, err, message)
  use var_lookup,     only:ixVarSoilData,nVarSoilData   ! index of soil data variables and number of variables 
  implicit none

  type(namevar), intent(in)     :: sdata(:)      ! soil data container for all the soil polygons
  integer(i4b),  intent(in)     :: polyLocal(:)    ! list of ID (=index) of soil polygons contributing model hru
  type(namevar), intent(inout)  :: sdataLocal(:) ! soil data container for local soil polygon 
  integer(i4b)                  :: nPolyLocal 
  integer(i4b)                  :: ivar 
  integer(i4b)                  :: iPoly

  ! Initialize error control
  err=0; message='subset_soil/'

  nPolyLocal = size(polyLocal)
  nSlyrs     = size(sdata(ixVarSoilData%sand)%dvar2,1)
        ...
        case('integer')
          select case(sdata_meta(iVar)%vardims)
            case('2D')
              ...
              if(ierr/=0)then; message=trim(message)//'problem allocating 2D int space for sdata data structure'; return; endif
              ! subset the data
              sdataLocal(ivar)%ivar2(:,iPoly)=sdata(ivar)%ivar2(:,polyLocal(iPoly))
            case('1D')
              ! allocate space for the 1D integer array 
              allocate(sdata(ivar)%ivar1(nPolyLocal),stat=ierr)
              if(ierr/=0)then; message=trim(message)//'problem allocating 1D int space for sdata data structure'; return; endif
              ! get the data
              sdataLocal(ivar)%ivar1(iPoly)=sdata(ivar)%ivar2(polyLocal(iPoly))
            end select
        case('double')
          select case(sdata_meta(iVar)%vardims)
            case('2D')
            ...
            case('1D')
              ...
              if(ierr/=0)then; message=trim(message)//'problem allocating 1D real space for sdata data structure'; return; endif
              ! subset the data
              sdataLocal(ivar)%dvar1(iPoly)=sdata(ivar)%dvar2(polyLocal(iPoly))
          end select
      end select 
    enddo
  enddo
